...
2. **Execution point.** In `review._refresh_running()`, after PROPOSAL.md exists and is committed to the branch (ADR-034), but before the status transitions to `done`. The verification runs in the project directory (not the worktree), which is the correct CWD for project-level build/test commands. Timeout: 300 seconds. Output truncated to 3000 characters.
